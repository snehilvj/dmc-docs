...
The new  `CopyButton` and `CustomCopyButton` components is like `dcc.Clipboard` with a Mantine theme.

- Customizable icons, colors, and labels for copied state
- Support for copying from another component using `target_id`
- Trigger copy in callbacks with `triggerCopy=True`
...
Need a quick way to say “no”? Fetch a random answer from [No-as-a-Service](https://github.com/hotheadhacker/no-as-a-service) and copy it with a single click.
