...
`MantineHooks`, `MantineCore`, and `MantineDates` are now exported from dash-mantine-components, giving component authors access to the same hooks and utilities DMC uses internally.
...
For more details and working examples, see custom DMC components inspired by [Mantine’s custom Combobox examples](https://mantine.dev/combobox/?e=BasicSelect) in this [GitHub repo](https://github.com/AnnMarieW/dmc_custom_components).
