...
Check out all the new features in the [Table Of Contents docs](/components/tableofcontents).
...
We now export `MantineHooks`, `MantineCore`, and `MantineDates` from dash-mantine-components. This gives component 
authors access to the same Mantine hooks and utilities that DMC uses internally.

With these exports, you can build more advanced components using the patterns recommended by Mantine. For example,
you can create a custom `Select` component on top of lower-level building blocks like `Combobox`, while keeping 
consistent theming in Dash.

See custom DMC components based on the [Mantine custom Combobox examples](https://mantine.dev/combobox/?e=BasicSelect) in this [GitHub](https://github.com/AnnMarieW/dmc_custom_components)
